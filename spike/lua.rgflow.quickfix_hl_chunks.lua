...
local function remove_zs_ze_chars(zs_ze, line, cwd)
    local rel_line = utils.remove_prefix(line, cwd)
    local zs = rel_line:find(zs_ze, 1)
    if not zs then
...
local function processChunk()
    ...
    for i = start_idx, end_idx do
        local line = STATE.results[i]
        local new_line, zs, ze = remove_zs_ze_chars(zs_ze, line, cwd)
        print(zs, '===>', ze, new_line)
        table.insert(chunk_lines, new_line)
        STATE.hi_info[i] = {
            zs = zs, ze = ze
        }
        -- {unpack(STATE.results, start_idx, end_idx)}
    end
    -- vim.print(STATE.hi_info)
